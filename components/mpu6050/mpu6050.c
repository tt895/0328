...
#define I2C_MASTER_TIMEOUT_MS 500  // 从100改为500，增加容错
//vcc-3v3
//SDA-8
//SCl-9
//GND-GND
...
static esp_err_t mpu6050_write_byte(uint8_t reg, uint8_t data)
{
    for (int retry = 0; retry < 3; retry++) {
        ...
        ESP_LOGW(TAG, "I2C写入失败（尝试%d/3），reg=0x%02X, 错误:%d", retry + 1, reg, ret);
...
static esp_err_t mpu6050_read_bytes(uint8_t reg, uint8_t *data, size_t len)
{
    ...
    for (int retry = 0; retry < 3; retry++) {
        ...
        ESP_LOGW(TAG, "I2C读取失败（尝试%d/3），reg=0x%02X, 错误:%d", retry + 1, reg, ret);
        vTaskDelay(pdMS_TO_TICKS(5));
    }
...
{
    if (initialized) {
        ESP_LOGW(TAG, "MPU6050已初始化");
        return ESP_OK;
    }

    ESP_LOGI(TAG, "初始化I2C: SDA=%d, SCL=%d, Freq=%d", sda_pin, scl_pin, i2c_freq);
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "唤醒MPU6050失败，请检查硬件连接");
    ...
    ret = mpu6050_write_byte(REG_GYRO_CONFIG, 0x00);
    if (ret != ESP_OK) {
        ESP_LOGW(TAG, "配置陀螺仪失败");
    }
    
    ret = mpu6050_write_byte(REG_ACCEL_CONFIG, 0x00);
    if (ret != ESP_OK) {
        ESP_LOGW(TAG, "配置加速度计失败");
    }

    uint8_t who_am_i;
    ret = mpu6050_read_bytes(REG_WHO_AM_I, &who_am_i, 1);
    if (ret != ESP_OK || (who_am_i != 0x68 && who_am_i != 0x72)) {
        ESP_LOGW(TAG, "MPU6050 ID验证失败(读到0x%02X)，但继续运行", who_am_i);
    } else {
        ESP_LOGI(TAG, "MPU6050 ID验证通过: 0x%02X", who_am_i);
    }
...
esp_err_t mpu6050_reset(void)
{
    ...
    ESP_LOGW(TAG, "正在执行MPU6050软件复位...");
    
    // 写入复位位（0x6B寄存器第7位）
    esp_err_t ret = mpu6050_write_byte(REG_PWR_MGMT_1, 0x80);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "复位命令发送失败");
        return ret;
    }
    
    // 等待复位完成（datasheet要求至少50ms）
    vTaskDelay(pdMS_TO_TICKS(100));
    
    // 重新初始化配置
    ret = mpu6050_write_byte(REG_PWR_MGMT_1, 0x00);  // 唤醒
    if (ret != ESP_OK) return ret;
    
    vTaskDelay(pdMS_TO_TICKS(10));
    
    ret = mpu6050_write_byte(REG_GYRO_CONFIG, 0x00);  // ±250deg/s
    if (ret != ESP_OK) return ret;
    
    ret = mpu6050_write_byte(REG_ACCEL_CONFIG, 0x00); // ±2g
    if (ret != ESP_OK) return ret;
    
    // 重置姿态角，让传感器从当前位置重新开始积分
    pitch = roll = yaw = 0;
    last_update_time = 0;
