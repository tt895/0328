...
{
    if (client_sock < 0 || data == NULL || len == 0) return -1;
    return send(client_sock, data, len, 0);
}

int wifi_manager_recv_tcp_data(int client_sock, void* buffer, size_t len)
{
    if (client_sock < 0 || buffer == NULL || len == 0) return -1;
    
    // 如果是非阻塞模式，返回0只是表示暂时没有数据
    int received = recv(client_sock, buffer, len, 0);
    ...
    if (received < 0) {
        ...
        ESP_LOGW(TAG, "接收错误: %d", err);
        return -1;
    }
    
    return received;  // 返回0表示对方关闭连接
}
